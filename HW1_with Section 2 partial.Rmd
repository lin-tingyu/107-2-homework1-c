...
請填入以下空格完成完整偏誤論述：  
有控制**經原PR**時，擋修會使得**個經PR** (1)_上升__(上升/下降) (2)__6.8__百分點，其值比未控制**經原PR**時還 (3)__低_(高/低)；這表示忽略**經原PR**會對效應係數估計產生 (4)正向偏誤_(正向偏誤(upward bias)/負向偏誤(downward bias))。之所以會有上述的偏誤方向來自於兩個因素：

  1. **經原PR**與**個經PR**有(5)_正向__(正向/反向)關連
  2. **可修個體**=1的族群，其**經原PR**平均較(6)_高_(高/低)
  
故若不控制**經原PR**，因**可修個體**=1與=0兩群人也會分別帶有 **經原PR**  (7)_高與低__(高與低/低與高)特質，所以**可修個體**=1的人其**個經PR**高，有部份是因其**經原PR**也較(8)高(高/低)所至。
